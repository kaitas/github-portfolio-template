...
公開されたサイトの `wizard.html` を開くと、質問に答えながらポートフォリオを作れます。
...
- 完成した `index.html` を保存し、GitHub上の同名ファイルと置き換えます
- `portfolio.json` を保存すると、あとからウィザードへ読み込んで編集を再開できます
- 本名、学籍番号、住所、電話番号、学校のメールアドレスは入力しないでください

HTMLを直接編集したい場合は、以下の手順でも作成できます。
